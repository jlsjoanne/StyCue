...
namespace Stycue.Api.Controllers
{
    ...
    /// </summary>
    /// <remarks>
    /// 提供委託文或分享/提問貼文留言列表、建立根留言、回覆留言、編輯留言與刪除留言功能
    /// </remarks>
    [Route("api")]
    [ApiController]
    [Tags("Comments")]
    public class CommentController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetPostComment(int postId, CancellationToken cancellationToken)
        {
            var userId = User.GetUserIdOrNull();

            var result = await _commentService.GetPostCommentsAsync(userId, postId, cancellationToken);
        ...
        /// <response code="400">請求內容驗證失敗，或貼文 ID、留言目標、圖片資料不符合規則。</response>
        /// <response code="401">尚未登入，或登入資訊無效。</response>
        /// <response code="403">沒有權限使用部分圖片。</response>
        /// <response code="404">找不到指定的貼文或圖片。</response>
        ...
        public async Task<IActionResult> CreatePostComment(
            int postId, [FromBody] UpsertCommentRequest request, CancellationToken cancellationToken)
        {
            var userId = User.GetUserId();

            var result = await _commentService.CreateForPostAsync(userId, postId, request, cancellationToken);
        ...
        /// 可未登入查詢。若使用者已登入，回應會包含是否為留言作者、是否可編輯、是否可刪除與是否已按讚。
        /// 回傳根留言列表，並包含第一層回覆留言。
        /// 已提前關閉的委託只有委託建立者可查詢留言；其他使用者會視為找不到指定委託文。
        /// </remarks>
        ...
        /// <response code="401">尚未登入，或登入資訊無效。</response>
        /// <response code="403">委託建立者不可對自己的委託建立根留言，或沒有權限使用部分圖片。</response>
